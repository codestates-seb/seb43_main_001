...
public class PortfolioCommentService {
    ...
    public PortfolioCommentDto.Response createPortfolioComment(PortfolioCommentDto.Post postDto) {
        PortfolioComment portfolioComment = portfolioCommentMapper
                .postToEntity(postDto);
        PortfolioComment savedComment = portfolioCommentRepository
                .save(setUserAndPortfolio(portfolioComment));
     ...
     * @return
     */
    public PortfolioCommentDto.Response updatePortfolioComment(
            PortfolioCommentDto.Patch patchDto
    ) {
        findVerifiedPortfolioComment(patchDto.getPortfolioCommentId());

        PortfolioComment portfolioComment = portfolioCommentMapper
                .patchToEntity(patchDto);
    ...
    public PortfolioCommentDto.ResponseList findPortfolioCommentsByUser(Long userId, int page, int size) {
        ...
        Page<PortfolioCommentDto.Response> portfoliosPage = portfolioCommentRepository
    ...
    public PortfolioCommentDto.ResponseList findPortfolioCommentsByPortfolio(Long portfolioId, int page, int size) {
        ...
        Page<PortfolioCommentDto.Response> portfoliosPage = portfolioCommentRepository
                .findByPortfolio(portfolio, pageable)
                .map(portfolioCommentMapper::entityToResponse);
    ...
    private PortfolioComment findVerifiedPortfolioComment(Long portfolioCommentId) {
        Optional<PortfolioComment> optionalPortfolioComment =
                portfolioCommentRepository.findById(portfolioCommentId);
