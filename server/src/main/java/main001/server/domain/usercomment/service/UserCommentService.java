...
public class UserCommentService {
    ...
    public UserCommentDto.ResponseList findUserCommentsByUser(Long userId, int page, int size) {
        ...
        Page<UserCommentDto.Response> usersPage = userCommentRepository.findByUser(user,pageable).map(userCommentMapper::entityToResponse);
        List<UserCommentDto.Response> content = usersPage.getContent();
        return new UserCommentDto.ResponseList(content,page,size, (int) usersPage.getTotalElements(),usersPage.getTotalPages());
    }
    ...
    public UserCommentDto.ResponseList findUserCommentsByWriter(Long writerId, int page, int size) {
        ...
        Pageable pageable = PageRequest.of(page,size);
        Page<UserCommentDto.Response> writersPage = userCommentRepository.findByWriter(writer,pageable).map(userCommentMapper::entityToResponse);
        List<UserCommentDto.Response> content = writersPage.getContent();
        return new UserCommentDto.ResponseList(content,page,size, (int) writersPage.getTotalElements(),writersPage.getTotalPages());
    }
    ...
    public UserComment findVerifiedUserComment(Long userCommentId) {
        ...
        return optionalUserComment.orElseThrow(() -> new RuntimeException("조회된 댓글이 없습니다."));
    }
